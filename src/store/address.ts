...
export const useAddress = defineStore('address', {
  state: () => ({
    address: JSON.parse(uni.getStorageSync('address') || '{}') as
      | UniApp.ChooseAddressRes
      | undefined,
    token: uni.getStorageSync('token'),
    userInfo: JSON.parse(uni.getStorageSync('userInfo') || '{}') as
      | UniApp.GetUserInfoRes['userInfo']
      | undefined,
